...
(List<Element> elements, int new_i) parse_md(List<int> runes, int i_in, {required bool return_on_newline}) {
	...
	outer_loop: for (; i < runes.length; i += 1) {
		...
		case HTML_Open:
			/* zips through html */
			for (; runes.nat(i, HTML_Close); i += 1)
				add(line, runes[i]);
		...
		case Underscore: /* italics */
			...
			i += 1;
			add_as_T(paragraph_content, line);
			for (; runes.nat(i, italic_start); i += 1)
				add(line, runes[i]);
			paragraph_content.add(Em(children: [T(line.toString())]));
			line.clear();
		case Pipe: /* table....... */
			...
			while (!break_row) {
				...
				for (; runes.nat(i, Pipe); i += 1) {
					int c = runes[i];
					if (c == Colon && i == begin) begin_colon = true;
					else if (c == Colon) end_colon = true;
					if (c == Newline) {
						break_row = true;
						break;
					}
			...
			for (int ii = 0; ii < column_alignments.length; ii += 1) {
				...
				case Alignment.Left: style_string = "text-align: left";
				case Alignment.Middle: style_string = "text-align: center";
				case Alignment.Right: style_string = "text-align: right";
				}
				headers.add(TableHeader(children: [T(top_row[ii])], inlineStyles: [style_string]));
			}
			// print("${headers}");
			elements.add( Table(head:
				TableHead(rows: [TableRow(cells: headers)]),
				body: TableBody(rows: [])
			));
...
(String href, Element content, int new_i) parse_link(List<int> runes, int i_in) {
	...
	if (runes.at(i, Bang)) {
		...
		for (; runes.nat(i, Bracket_Close); i += 1)
		add(line, runes[i]);
		content = T(line.toString());
	}
	line.clear();
	i += 2;
	for (; runes.nat(i, Paren_Close); i += 1)
		add(line, runes[i]);
